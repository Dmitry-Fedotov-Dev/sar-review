...
def test_slow_playback_stays_available():
    """Замедление -- требование методики отсмотра, и оно обязано остаться
    доступным.

    Раньше оно жило в нативном меню "⋮", и тест следил, чтобы это меню не
    спрятали. Меню всё же убрано -- иначе невозможно предсказать, где оно
    кончается, и своя кнопка полного экрана на него наезжала. Сама
    скорость переехала в свой список рядом с кнопкой, поэтому проверяем
    доступность, а не конкретное меню.
    """
    html = rendered()
    assert 'id="speed"' in html, "выбор скорости пропал"
    for value in ("0.25", "0.5", "0.75"):
        assert f'value="{value}"' in html, f"нет скорости {value}"
