...
<div class="wrap">
  ...
  <div class="masthead">
    ...
    <p>Короткий гид по работе с системой: вход, автоматический отчёт, ручная разметка, координаты. Прочитайте раздел 4 обязательно, до того как начнёте передавать координаты дальше.</p>
  </div>
  ...
  <section class="step" id="login">
    <div class="step-head"><span class="step-num">01</span><h2>Вход в систему</h2></div>
  ...
  <section class="step" id="list">
    <div class="step-head"><span class="step-num">02</span><h2>Список файлов</h2></div>
  ...
  <section class="step" id="scenes">
    <div class="step-head"><span class="step-num">03</span><h2>Автоматический отчёт</h2></div>
  ...
  <section class="step" id="coords">
    <div class="step-head"><span class="step-num">04</span><h2>Координаты</h2></div>
  ...
  <section class="step" id="telemetry">
    <div class="step-head"><span class="step-num">05</span><h2>Полная телеметрия кадра</h2></div>
  ...
  <section class="step" id="player">
    <div class="step-head"><span class="step-num">06</span><h2>Ручной плеер и разметка</h2></div>
  ...
  <section class="step" id="upload">
    <div class="step-head"><span class="step-num">07</span><h2>Загрузка файлов</h2></div>
  ...
  <section class="step" id="help">
    <div class="step-head"><span class="step-num">08</span><h2>Если что-то не работает</h2></div>
    <details class="faq">
...
const SETUP_HTML = `
  <div class="setup">
    ...
    <ol>
      <li>Поднять <code>mediamtx</code> на этой же машине или рядом. Он принимает
          поток с дрона/ноутбука в поле и раздаёт его браузерам:
        <pre>1935 — приём RTMP    rtmp://сервер:1935/bort2
8890 — приём SRT     устойчивее на плохой связи в горах
8888 — раздача HLS   это адрес для настройки ниже</pre></li>
      <li>Прописать адрес HLS в <code>sar_config.json</code>:
        <pre>"server": {{
  "stream_server_url": "http://адрес-сервера:8888"
}}</pre></li>
      <li>Вещающий клиент объявляет поток запросом
          <code>POST /api/streams/bort2/announce</code> с телом
          <code>{{"title": "Борт 2, южный склон"}}</code> и дальше раз в
          10-15 секунд шлёт <code>/heartbeat</code>. Без heartbeat поток сам
          уйдёт в офлайн — чтобы никто не ждал картинку, которой уже нет.</li>
    </ol>
    <p style="color:#888">Ключ потока в адресе (<code>bort2</code>) должен совпадать
    с тем, куда вещаете в mediamtx — по нему платформа и строит ссылку
    воспроизведения.</p>
