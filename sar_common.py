...
DEFAULT_SERVER_CONFIG = {
    ...
    # Для mediamtx это адрес его HLS-порта, по умолчанию 8888:
    #     "stream_server_url": "http://192.168.1.50:8888"
    "stream_server_url": "",
    # Шаблон ссылки воспроизведения; {key} подставляется из stream_key.
    #
    # Значение по умолчанию -- ровно то, что отдаёт mediamtx по HLS.
    # HLS работает во всех браузерах без плагинов, но даёт задержку 5-15с.
    # Если задержка критична, у mediamtx есть WebRTC почти без задержки --
    # тогда порт 8889 и шаблон "{server}/{key}/whep" (браузеры его понимают
    # через JS-клиент, обычный <video> с ним не работает напрямую).
    #
    # Порты mediamtx для справки:
    #   1935 -- приём RTMP (rtmp://сервер:1935/<ключ>)
    #   8890 -- приём SRT (устойчивее RTMP на плохой связи в горах)
    #   8888 -- раздача HLS   <- это и нужно здесь
    #   8889 -- раздача WebRTC
